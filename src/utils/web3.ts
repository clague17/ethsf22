...
export const getIsSupportedChain = (chainId: number, isDev: boolean): boolean => {
...
export const getSupportedChains = (isDev: boolean): SpiralsChain[] => {
  ...
  if (isDev) {
    supportedChains = supportedChains.concat(...Array.from(TESTNET_CHAINS.values()));
  }
...
export const getChainType = (
  ...
  return _.find(getSupportedChains(isDev), (x: SpiralsChain) => x.id === chainID)?.type;
};
...
export const TESTNET_CHAINS: Map<number, SpiralsChain> = new Map([
  ...
  [
    MUMBAI_TESTNET_CHAIN_ID,
    {
      ...
      network: "https://polygon-mumbai.g.alchemy.com/v2/FBLKdjji7knNPlTxO7QWoxA9S2RcCeV3",
      rpcUrls: {
        default:
